...
parser.add_argument('-e',
           '--extra-package-path',metavar='extra_package_path',help='Add extra packet(clients, modules, protocols) to ICSsploit.')


def icssploit(extra_package_path=package_path):
    if not os.path.isdir(extra_package_path):
        extra_package_path = None
    icssploit = IcssploitInterpreter(extra_package_path)
    icssploit.start()
